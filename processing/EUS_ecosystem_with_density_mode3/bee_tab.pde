...
void setCohesion(float coh) {
  ...
  // Modes 1 and 2 use the familiar central drift. Mode 3 supplies a position
  // calculated from remembered audience direction plus autonomous movement.
  float systemX =
    width * 0.5f +
    sin(frameCount * 0.006f) * width * 0.12f;

  if (finalSlider == 3 && mode3SystemX > 0.0f) {
    systemX = mode3SystemX;
  }
