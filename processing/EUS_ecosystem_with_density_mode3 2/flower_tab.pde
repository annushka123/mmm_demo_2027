...
void display() {
  // Mode 1 follows the audience directly. Mode 3 uses its slower remembered
  // climate. Mode 2 remains neutral.
  float activeHeat = 0.0f;
  float activeStorm = 0.0f;

  if (finalSlider == 1) {
    activeHeat = heatStrength;
    activeStorm = stormStrength;
  }

  if (finalSlider == 3) {
    activeHeat = mode3HeatStrength;
    activeStorm = mode3StormStrength;
  }
