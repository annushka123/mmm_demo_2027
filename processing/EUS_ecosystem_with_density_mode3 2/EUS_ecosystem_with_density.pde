...
ArrayList<Float> audienceSpeedMemory = new ArrayList<Float>();
ArrayList<Float> horizontalMemory = new ArrayList<Float>();

// Wekinator is currently sending roughly 30 messages per second, so 300
// samples gives Mode 3 approximately ten seconds of recent behaviour.
int maxMemorySize = 300;
color currentBG;
color targetBG;
ArrayList<Float> densityMemory = new ArrayList<Float>();

// Historical name retained so the rest of the sketch remains compatible.
// This contains violin pitch classes, not the audience's bow position.
