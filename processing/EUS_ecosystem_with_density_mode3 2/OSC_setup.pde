...
if (
  ...
  if (theOscMessage.checkAddrPattern("/max/outputs/buttons") == true) {
    ...
    println("button: " + buttonID);
      switch (buttonID) {
    case 5: // Button 3 short press is now reserved for calibration.
      useAutonomousSlider = false;
      println("Button 3: calibration control (hold for two seconds)");
      break;

    case 6: // Arduino performs calibration locally after the long hold.
      useAutonomousSlider = false;
      println("Button 3 held: Arduino calibration should be blue");
      break;
